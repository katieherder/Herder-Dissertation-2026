...
tian_data_clean <- tian_data %>%
  ...
  # Standardize dose: subtract mean and divide by SD
  group_by(agent) %>% 
  mutate(dose_scaled = (dose - mean(dose, na.rm = TRUE)) / sd(dose, na.rm = TRUE)) %>%
  ungroup()
...
multi_fun <- dmulti(list(
  ...
  AE  = duser(fun = quad_fun, beta.1 = "rel", beta.2 = "rel"),
  MBE = duser(fun = linear_fun, beta.1 = "rel"),
  ME  = duser(fun = linear_fun, beta.1 = "rel"),
