...
#   1. Network setup (fluoxetine-equivalent dosing)
...
#   7. Prior sensitivity analysis on tau
...
lumped_pD <- model_Lumped$jagsresult$BUGSoutput$DIC -
  model_Lumped$jagsresult$BUGSoutput$mean$deviance
...
# Deviance-deviance plot (Model A vs Ref)
jpeg("figures/Figure_devdev.jpeg", width = 8, height = 6, units = "in", res = 300)
...
placebo_rate <- sum(ssri$r[ssri$agent == "placebo"]) /
  sum(ssri$n[ssri$agent == "placebo"])
...
jpeg("figures/Figure 4.5.jpeg", width = 10, height = 6, units = "in", res = 300)
plot(pred_A) + ggtitle("Model A: Shared Emax")
dev.off()

jpeg("figures/Figure 4.6.jpeg", width = 10, height = 6, units = "in", res = 300)
plot(pred_B) + ggtitle("Model B: Agent-specific Emax")
dev.off()

jpeg("figures/Figure 4.7.jpeg", width = 10, height = 6, units = "in", res = 300)
plot(pred_Ref) + ggtitle("Ref: Shared Linear")
dev.off()

# Fitted values
jpeg("figures/Figure 4.4.jpeg", width = 10, height = 8, units = "in", res = 300)
fitplot(model_A)
dev.off()

jpeg("figures/Figure_fitplot_B.jpeg", width = 10, height = 8, units = "in", res = 300)
fitplot(model_B)
dev.off()

jpeg("figures/Figure_fitplot_Ref.jpeg", width = 10, height = 8, units = "in", res = 300)
fitplot(model_Ref)
dev.off()
...
#
# This ensures cross-model comparisons are on the same scale.
# ==============================================================================

agent_names <- c("citalopram", "escitalopram", "fluoxetine",
...
print(data.frame(agent = names(max_rank_doses),
                 max_dose = unlist(max_rank_doses)))
...
ranking_results_max <- bind_rows(ranks_A_max, ranks_B_max,
                                 ranks_Ref_max, ranks_Lmp_max)

cat("\nModel A:\n")
print(ranks_A_max[order(ranks_A_max$mean_rank),
                  c("agent", "mean_rank", "sucra", "d_median")])
cat("\nModel B:\n")
print(ranks_B_max[order(ranks_B_max$mean_rank),
                  c("agent", "mean_rank", "sucra", "d_median")])
cat("\nRef: Linear:\n")
print(ranks_Ref_max[order(ranks_Ref_max$mean_rank),
                    c("agent", "mean_rank", "sucra", "d_median")])
cat("\nLumped NMA:\n")
print(ranks_Lmp_max[order(ranks_Lmp_max$mean_rank),
                    c("agent", "mean_rank", "sucra", "d_median")])
...
# Order agents by Model B SUCRA (highest first)
model_b_order <- ranking_results_max %>%
  filter(model == "Model B") %>%
  arrange(desc(sucra)) %>%
  pull(agent)

ranking_results_max$agent <- factor(ranking_results_max$agent,
                                    levels = rev(model_b_order))

p_sucra <- ggplot(ranking_results_max,
                  aes(x = agent, y = sucra, fill = model)) +
...
print(p_sucra)
ggsave("figures/Figure 4.8.jpeg", p_sucra,
...
print(p_tau_prior)
ggsave("figures/Figure_tau_posteriors.jpeg", p_tau_prior,
       width = 9, height = 5, dpi = 300)

# SUCRA stability
p_sucra_prior <- ggplot(sucra_by_prior,
                        aes(x = prior, y = sucra, color = agent, group = agent)) +
  geom_point(size = 3) +
...
print(p_sucra_prior)
ggsave("figures/Figure_SUCRA_prior_stability.jpeg", p_sucra_prior,
...
ggsave("figures/Figure_DIC_LPML_priors.jpeg", p_fit_combined,
...
# ---- CSVs ----
write.csv(comparison_table, "results/model_comparison.csv", row.names = FALSE)
...
write.csv(convergence_df, "results/convergence_diagnostics.csv", row.names = FALSE)

write.csv(sucra_wide, "results/sucra_comparison_max_dose.csv", row.names = FALSE)
write.csv(ranking_results_max, "results/ranking_results_full.csv", row.names = FALSE)

max_dose_df <- data.frame(agent = names(max_rank_doses),
                          max_equiv_dose = unlist(max_rank_doses))
write.csv(max_dose_df, "results/max_equiv_doses.csv", row.names = FALSE)

lumped_results <- ranks_Lmp_max[, c("agent", "d_median", "d_lower", "d_upper",
                                    "mean_rank", "sucra")]
write.csv(lumped_results, "results/lumped_nma_results.csv", row.names = FALSE)
...
write.csv(dr_params, "results/model_B_dr_parameters.csv", row.names = FALSE)
...
write.csv(tau_summary, "results/heterogeneity_tau.csv", row.names = FALSE)

write.csv(prior_summary, "results/prior_sensitivity_summary.csv", row.names = FALSE)
write.csv(sucra_by_prior, "results/prior_sensitivity_sucra.csv", row.names = FALSE)

write.csv(rank_stability_A, "results/rank_stability_model_A.csv", row.names = FALSE)
write.csv(rank_stability_B, "results/rank_stability_model_B.csv", row.names = FALSE)
...
save(model_A, model_B, model_Ref, model_Lumped,
     ...
     file = "results/ssri_all_models.RData")
