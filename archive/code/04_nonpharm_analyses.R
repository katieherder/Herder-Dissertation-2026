...
# For Lumping - aggregate duplicate treatment arms within studies
...
any(duplicated(lumped_data[, c("studyID", "agent")]))  # should be FALSE

print(paste("MBNMAdose rows:", nrow(mbnma_data)))
print(paste("Lumping rows:", nrow(lumped_data)))
print(paste("Lumping studies:", length(unique(lumped_data$studyID))))

# For gemtc (lumping) - Network Plot
gemtc_data <- Tian %>%
...
#==============================================================================
# METHOD 1: LUMPING (MBNMAdose nma.run with link = "smd")
# Standard NMA ignoring dose - each modality is one node
# Uses MBNMAdose for SMD-scale comparability with other models
#==============================================================================
...
lumped_bugs <- result_lumped$jagsresult$BUGSoutput
cat("\nLumping DIC:", lumped_bugs$DIC, "\n")
cat("Lumping pD:", lumped_bugs$pV, "\n")
cat("Lumping ResidDev:", lumped_bugs$summary["totresdev", "mean"], "\n")
cat("Lumping tau:", lumped_bugs$summary["sd", "50%"], "\n")

# Convergence
lumped_rhat <- lumped_bugs$summary[, "Rhat"]
lumped_ess <- lumped_bugs$summary[, "n.eff"]
cat("\nMax Rhat:", max(lumped_rhat, na.rm = TRUE), "\n")
cat("Min ESS:", min(lumped_ess[lumped_ess > 1], na.rm = TRUE), "\n")

# Store results
results_all$lumping <- list(
  model = result_lumped,
  network = network_lumped
)

# Store convergence
convergence_all <- rbind(convergence_all, data.frame(
  Method = "Lumping",
  MaxRhat = max(lumped_rhat, na.rm = TRUE),
  MinESS = min(lumped_ess[lumped_ess > 1], na.rm = TRUE),
  Converged = max(lumped_rhat, na.rm = TRUE) < 1.05
))
...
convergence_all <- rbind(convergence_all, data.frame(
  ...
  MinESS = min(split_ess[split_ess > 1], na.rm = TRUE),
...
#==============================================================================
# MODEL COMPARISON TABLE (Table 3.X) - Primary 5 models only
#==============================================================================
cat("\n\n========== MODEL COMPARISON (Primary Analysis) ==========\n")

# Function to extract DIC components from MBNMAdose MBNMA models
...
# Build comparison table — 5 primary models only
model_names <- c("Splitting", "Linear MBNMA", 
                 "Emax MBNMA", "Quadratic MBNMA", "NCS MBNMA")
model_comparison <- data.frame(Method = model_names)

# --- Splitting ---
split_bugs <- results_all$splitting$model$jagsresult$BUGSoutput
model_comparison$DIC[1] <- split_bugs$DIC
model_comparison$pD[1] <- split_bugs$pV
model_comparison$ResidDev[1] <- split_bugs$summary["totresdev", "mean"]
model_comparison$tau[1] <- split_bugs$summary["sd", "50%"]

# --- Linear ---
fit <- extract_fit_mbnma(results_all$linear$model)
model_comparison[2, c("DIC","pD","ResidDev","tau")] <- c(fit$DIC, fit$pD, fit$resdev, fit$tau)
...
model_comparison$DIC[4] <- quad_bugs$DIC
model_comparison$pD[4] <- quad_bugs$pD
model_comparison$ResidDev[4] <- quad_bugs$summary["totresdev", "mean"]
model_comparison$tau[4] <- quad_bugs$summary["sd", "50%"]

# --- NCS ---
fit <- extract_fit_mbnma(results_all$rcs$model)
model_comparison[5, c("DIC","pD","ResidDev","tau")] <- c(fit$DIC, fit$pD, fit$resdev, fit$tau)
...
# Delta DIC — all 5 models on same SMD scale, 45 studies, fully comparable
model_comparison$DeltaDIC <- round(
  model_comparison$DIC - min(model_comparison$DIC, na.rm = TRUE), 1)

cat("\nPrimary Model Comparison (45 studies, SMD scale):\n")
print(model_comparison)


#==============================================================================
# LUMPED NMA SUMMARY — Benchmark vs Tian et al.
#==============================================================================

cat("\n===== LUMPED NMA (40-study benchmark) =====\n")

bugs <- results_all$lumping$model$jagsresult$BUGSoutput
summ <- bugs$summary
...
cat("tau =", round(tau, 3), "\n")
cat("tau^2 =", round(tau2, 3), "\n")

# 2) Treatment effects vs control (drop d[1] = CON)
d <- summ[grep("^d\\[", rownames(summ)), ]
d <- d[-1, ]
...
results_lumped <- data.frame(
  ...
  SMD   = round(d[, "50%"], 2),
  Lower = round(d[, "2.5%"], 2),
  Upper = round(d[, "97.5%"], 2)
)

# Order by effect size (most negative = best)
results_lumped <- results_lumped[order(results_lumped$SMD), ]
...
cat("Total studies:", length(unique(Tian$Study)), "\n")
cat("Lumping studies:", length(unique(lumped_data$studyID)), "\n")
cat("Splitting studies:", length(unique(split_data$studyID)), "\n")
cat("MBNMA studies:", length(unique(mbnma_data$studyID)), "\n")

library(coda)
sims <- results_all$emax$model$BUGSoutput$sims.array

# Plot 1: All 4 Emax parameters
...
jpeg("figures/trace_emax_emax.jpeg", width = 3000, height = 3200, res = 300)
par(mar = c(2, 2, 2, 1))
plot(mcmc_emax, ask = FALSE)
dev.off()

# Plot 2: All 4 ED50 parameters
...
jpeg("figures/trace_emax_ed50.jpeg", width = 3000, height = 3200, res = 300)
par(mar = c(2, 2, 2, 1))
...
jpeg("figures/trace_emax_sd.jpeg", width = 3000, height = 1800, res = 300)
par(mar = c(2, 2, 2, 1))
plot(mcmc_other, ask = FALSE)
dev.off()

#==============================================================================
# COMPARISON: Lumping vs Emax at Median Dose
#==============================================================================

cat("\n========== LUMPING vs EMAX AT MEDIAN DOSE ==========\n")

#==============================================================================
# COMPARISON: Lumping vs Emax (Primary & Sensitivity) at Median Dose
#==============================================================================

cat("\n========== LUMPING vs EMAX AT MEDIAN DOSE (PRIMARY + SENSITIVITY) ==========\n")

# 1. Primary: median MET-min/week
...
emax_at_median <- emax_preds %>%
  ...
  ungroup() %>%
  select(Agent, Dose_raw, Median, Lower, Upper)

# 2. Sensitivity: median min/week (back-transformed from scaled)
...
combined_comparison <- emax_at_median %>%
  select(Agent, SMD_Emax = Median, Lower_Emax = Lower, Upper_Emax = Upper) %>%
  left_join(
    emax_at_median_sens %>%
      select(Agent, SMD_Sens = Median, Lower_Sens = Lower, Upper_Sens = Upper),
    by = "Agent"
  ) %>%
  left_join(
    results_lumped %>%
      select(Modality, SMD_Lumped = SMD, Lower_Lumped = Lower, Upper_Lumped = Upper),
    by = c("Agent" = "Modality")
  ) %>%
  mutate(
  ...
  arrange(SMD_Emax) %>%
  select(Agent, Emax_MET, Emax_Min, Lumped)

cat("\n--- Comparison Table ---\n")
cat("Emax_MET = Primary Emax at median", round(median_dose_raw), "MET-min/week\n")
cat("Emax_Min = Sensitivity Emax at median", round(median_dose_min), "min/week\n")
