...
#
# CORRECTED VERSION — March 18, 2026
...
# scale) for ALL models (A, B, Ref, Lumped). The previous version used the
# MBNMAdose predict() → rank() pipeline for dose-response models, which
# ranked on absolute predicted probabilities (including a fixed baseline E0),
# while Lumped ranked on relative effects — causing systematic ranking
# inversion for dose-response models.
#
...
#   effects) using the same ranking mechanism. The previous version used the
#   MBNMAdose predict() → rank() pipeline for dose-response models, which
#   ranked on ABSOLUTE predicted probabilities (including a fixed baseline),
#   introducing a systematic scale mismatch vs the truth and vs Lumped.
# ==============================================================================
...
extract_metrics <- function(fitted_models, true_params, placebo_rate = 0.393,
  ...
  for (m in model_names) {
    ...
    # This is identical for A, B, Ref, and Lumped — all use d_hat.
...
writeLines(capture.output(sessionInfo()), "results/simulation/session_info.txt")